...
typedef struct{
	...
	byte 		data;		//el valor de datos de la celda, en una submatriz ya sea blanca o negra
	byte 		isGhost;	//dice si la celda es o no es de tipo ghost
	unsigned int 	ii;		//es el i en la matriz original
...
//se usa para DEBUG
//pero tambien se usa en el proceso. Es FUNDAMENTAL ESTA FUNCION!
...
void defineGhostCells(void){ //ex load_structs
	...
	for (i=0; i<Lmpix; i++){
		for (j=0; j<Lmpiy; j++){
			...
			ii=(((i+j)%2)*Lmpix+i)/2; 					 //ii, pertenece a alguna de las submatrices
...
//el agregado de if's dan la funcionalidad para topologias vectoriales de nodos, extension de las topologias matriciales
// con esta topologia puedo usar cantidad de nodos de la forma: 1x6, 1x5
...
static void update(const float temp, cell whites[Lmpix/2][Lmpiy], cell blacks[Lmpix/2][Lmpiy]) {
	...
	updateKernel(temp, WHITES, whites, blacks); 	// escribe blancas, lee de negras: params :<color, escribe, lee>
		...
		if (myrank_mpi==ROOT){
			FILE *fp = NULL;
			char _filename[40];
			sprintf(_filename, "%s%s%i%s%i%s%i%s", "pottsMPI_", "spinFlip_", L,"-", NODESX,"-", NODESY, ".txt");//para HYBR agregar num_threads
...
int main(int argc, char** argv){
	...
	if (myrank_mpi==ROOT){//-----------------------------------------ROOT-------------------------------------------------------------------
			...
			// parameters checking originales
			...
			assert(((L/NODESX)+2)%2==0);
				...
				if (myrank_mpi==ROOT){
					FILE *fp = NULL;
					char _filename[40];
					sprintf(_filename, "%s%s%i%s%i%s%i%s", "pottsMPI_", "spinFlip_", L,"-", NODESX,"-", NODESY, ".txt");
					fp = fopen(_filename, "a"); 
