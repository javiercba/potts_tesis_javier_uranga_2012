...
/* Major changes. New features were included
...
//#define FRAME 512
//#define FRAME 516
#define NUM_THREADS 32 //512 // 8 // 516 //(FRAME*FRAME/2)
...
unsigned int pasodebug;

//-------------------------------------------------------------DEBUG --------------------------------------------------------------------------------


void showBlackWhiteMatrix(){ //ex: do_file()

	FILE *fp = NULL;
	unsigned int i,j;
        char _filename[40];
	sprintf(_filename, "%s%s", "pottsSec", ".txt");
	fp = fopen(_filename, "a"); 

	fprintf(fp, "\nINICIO volcado de matriz\n");
	...
	for ( i=0; i<L/2; i++) {
		...
		for ( j=0; j<L; j++) {
			...
			fprintf(fp, "%u,  ", black[i][j]);
			
		}

		fprintf(fp, "\n\n");
	}
	fprintf(fp, "\nFIN volcado de matriz\n");
	fprintf(fp, "\n\n");
	fclose(fp);
}
...
void inverse_test(void){
	...
	fprintf(fp, "\nINICIO de OPERACION_INVERSA: vista de como estaria conformada la matriz ORIGINAL sin aplicar tecnica de blancas-negras\n");
		...
		for (i=0; i<L; i++){
			for (j=0; j<L; j++){
					...
					ii=(((i+j)%2)*L+i)/2; 
					if (ii<L/2)
						 fprintf(fp, "[%i,%i]= %i,", i,j, white[ii][j]);
					
					else
						 fprintf(fp, "[%i,%i]= %i,", i,j, black[ii-(L/2)][j]);
				
			}
			fprintf(fp, "\n\n");
		}

	fprintf(fp, "\nFIN de OPERACION_INVERSA: fin de la vista de la matriz ORIGINAL\n");
	fprintf(fp, "\n\n");				
	fclose(fp);
...
static double calculateKernel(byte white[L/2][L],
	...
	for (unsigned int i=0; i<L/2; i++) {
		for (unsigned int j=0; j<L; j++) {
			...
			spin_neigh_s = black[(i+(2*(j%2)-1)+L/2)%(L/2)][j]; //spin_neigh_s = black[(i+(2*(j%2)-1)*1+L/2)%(L/2)][j];
		...
		for (unsigned int j=0; j<TMAX; j++) {
			...
			if (j%calc_step==0) {
				...
				energy = calculateKernel(white, black, &M_max);
					...
					sprintf(_filename, "%s%s", "pottsSec", ".txt");
					fp = fopen(_filename, "a");
					fprintf(fp, "\n****ENERGY despues de calculatekernel y ANTES del acumulador:%f****\n", energy);
					fclose(fp);
					#endif
